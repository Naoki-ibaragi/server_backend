...
pub struct OperatingSummary{
    ...
    pub lot_start_time:NaiveDateTime,
    pub lot_end_time:NaiveDateTime,
    pub lot_time:f64, //ロット流動時間(s)
    pub start_time:f64, //ロット稼働中時間
    pub stop_time:f64, //ロット停止中時間
    pub stop_time_map:HashMap<String,f64>, //各要素毎のロット停止時間
    pub stop_count_map:HashMap<String,u64>, //各要素毎のロット停止回数
    pub alarm_detail_map:HashMap<String,String>
}
...
pub async fn get_operating_data(pool: &PgPool,machine_id:i32,start_date:&str,end_date:&str) -> Result<Vec<OperatingSummary>,Box<dyn Error>> {
    ...
    println!("dc1_alarm_map");
    println!("{:?}",dc1_alarm_map);

    //最初にlotdateテーブルからロットのstart_date~end_date内の該当設備のロット番号一覧を取得する
    let sql="SELECT lot_name FROM lotdate WHERE machine_id = $1 and start_date > $2::timestamp and end_date < $3::timestamp ORDER BY start_date ASC";
    ...
    for row in rows{
        ...
        for row in event_rows{
            ...
            let alarm_unit:Option<String>=row.get("alarm_unit");
            let alarm_code:Option<i32>=row.get("alarm_code");
            ...
            if event_type=="LOT_START"{
                ...
                current_alarm_code=alarm_code.unwrap_or(0);
                current_alarm_unit=alarm_unit.unwrap_or_else(|| String::new());
                ...
                *stop_count_map.entry(key.clone()).or_insert(0)+=1;
                ...
                if stop_state==true{
                    ...
                    if alarm_state==false{
                        ...
                        *stop_time_map.entry(key).or_insert(0.0)+=delta_sec;
...
fn load_alarm_map(
    ...
    let path_arr=[
        Path::new("./src/components/jp_alm_ld_utf8.txt"),
        Path::new("./src/components/jp_alm_dc1_utf8.txt"),
        Path::new("./src/components/jp_alm_ac1_utf8.txt"),
        Path::new("./src/components/jp_alm_ac2_utf8.txt"),
        Path::new("./src/components/jp_alm_dc2_utf8.txt"),
        Path::new("./src/components/jp_alm_ip_utf8.txt"),
        Path::new("./src/components/jp_alm_uld_utf8.txt")
    ];
    ...
    for i in 0..map_arr.len(){
        match read_alarm_map_data(map_arr[i],path_arr[i]) {
            Ok(_) => println!("Successfully loaded alarm map: {:?}", path_arr[i]),
            Err(e) => eprintln!("Failed to load alarm map {:?}: {}", path_arr[i], e),
        }
    }
